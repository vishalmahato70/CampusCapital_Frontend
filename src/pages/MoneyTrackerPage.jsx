...
function AccountsSection({ state, headers, fetchAllData }) {
  ...
  async function addAccount(e) {
    e.preventDefault();
    try {
      const res = await fetch(`${API_URL}/accounts`, { method: "POST", headers, body: JSON.stringify(form) });
      if (res.ok) {
        await fetchAllData();
  ...
  async function removeAccount(id) {
    try {
      const res = await fetch(`${API_URL}/accounts/${id}`, { method: "DELETE", headers });
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <SectionCard title="Add Account" right={<span className="text-xs text-slate-500">Bank, Card, or UPI/Wallet</span>}>
        <form onSubmit={addAccount} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <InlineInput label="Institution" placeholder="HDFC / SBI / ICICI" value={form.institution} onChange={(e) => setForm(f => ({ ...f, institution: e.target.value }))} />
          ...
          <div className="sm:col-span-2 flex justify-end"><button className="px-4 py-2 rounded-lg bg-[#00A7A7] text-white hover:bg-[#009090] text-sm">Add Account</button></div>
        </form>
      </SectionCard>
      <div className="lg:col-span-2">
        <SectionCard title="Accounts">
          {state.accounts.length === 0 ? <Empty title="No accounts yet" body="Add a bank account, credit card, or UPI/wallet to begin tracking." /> : (
              ...
              {state.accounts.map(a => (
                <div key={a._id} className="py-3 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button className="px-2 py-1 text-xs rounded-md border border-slate-200 hover:border-slate-300" onClick={() => removeAccount(a._id)}>Remove</button>
                  </div>
                </div>
              ))}
            </div>
...
function IncomeSection({ state, headers, fetchAllData }) {
    ...
    async function addIncome(e) {
        e.preventDefault();
        try {
            const res = await fetch(`${API_URL}/incomes`, { method: "POST", headers, body: JSON.stringify(form) });
            if (res.ok) {
                await fetchAllData();
    ...
    async function removeIncome(id) {
        try {
            const res = await fetch(`${API_URL}/incomes/${id}`, { method: "DELETE", headers });
            if (res.ok) await fetchAllData();
        } catch (err) { console.error(err); }
    }
...
function ExpensesSection({ state, headers, fetchAllData }) {
    ...
    async function addExpense(e) {
        e.preventDefault();
        try {
            const res = await fetch(`${API_URL}/expenses`, { method: "POST", headers, body: JSON.stringify(form) });
            if (res.ok) {
                await fetchAllData();
...
function SavingsSection({ state, headers, fetchAllData }) {
    ...
    async function addSaving(e) {
        e.preventDefault();
        try {
            const res = await fetch(`${API_URL}/savings`, { method: "POST", headers, body: JSON.stringify(form) });
            if (res.ok) {
                await fetchAllData();
    ...
    async function removeSaving(id) {
        try {
            const res = await fetch(`${API_URL}/savings/${id}`, { method: "DELETE", headers });
            if (res.ok) await fetchAllData();
        } catch (err) { console.error(err); }
    }
